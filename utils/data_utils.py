...
from quant.quant_layer import QuantMatMul, QuantModule, Union
...
def save_grad_data(model: QuantModel, layer: Union[QuantModule, BaseQuantBlock], cali_data: torch.Tensor,
                   act_quant: bool = False, batch_size: int = 32, keep_gpu: bool = True):
    """
    ...
    cached_grads = cached_grads.abs() + 1
...
class GetLayerInpOut:
    ...
    def __call__(self, model_input):
        ...
        with torch.no_grad():
            try:
                _ = self.model(model_input.to(self.device)) # I/O
...
class GetLayerGrad:
    ...
    def __call__(self, model_input):
        ...
        handle = self.layer.register_backward_hook(self.data_saver) # register_full_backward_hook?
...
def quantize_model_till(model: QuantModule, layer: Union[QuantModule, BaseQuantBlock], act_quant: bool = False, current_quant: bool = True):
    """
